...
# 2. Environment
cp .env.example .env
# fill in DATABASE_URL (point it at the local Postgres above), JWT_SECRET.
# Leave SUPABASE_* blank for local dev — photo storage falls back to local disk automatically.
...
# 4. Run — two processes, like before
npm run dev:api    # Express on :3000
npm run dev         # Vite on :5173, proxies /api/* to :3000 (see vite.config.ts)
```

Open `http://localhost:5173`. Demo accounts from the seed script:

| Role | Identifier | Password | Scenario |
|---|---|---|---|
| Admin | `admin.fauzi@fieldwork.id` | `admin123` | Full dashboard access |
| Teknisi | `budi.santoso@fieldwork.id` | `lapangan123` | Anti Rayap job in progress; a completed Pest Control job with full treatment record |
| Teknisi | `sinta.maharani@fieldwork.id` | `lapangan123` | Fumigasi job flagged: GPS out of radius **and** missing aeration data |
| Teknisi | `andi.pratama@fieldwork.id` | `lapangan123` | Ready to check in, recurring contract |

## Deploying (Vercel + Supabase)

1. **Supabase**: create a project. Storage → create a bucket (default name `evidence-photos`) and make it **private**. SQL Editor → paste and run `FINAL_MIGRATION_BUNDLE.sql` once. Note the Project URL, service_role key (Project Settings → API), and the Transaction pooler connection string, port 6543 (Project Settings → Database).
2. **Vercel**: `vercel link` in the repo root, then set the four core env vars (`vercel env add DATABASE_URL` etc. — use the pooler string for `DATABASE_URL`), then `vercel --prod`.
3. Run `npm run db:seed` once locally against the Supabase **direct** connection (port 5432) if you want the demo accounts in production too — otherwise just create your real admin account by hand (`insert into users (...) values (..., 'APPROVED', true)`) or sign up through the app and approve yourself via a direct SQL update the first time.
